...
class Lesson:
    ...
    def str_to_lesson(cls, s: str):
        date_match = re.search(r'(19|20)\d{2}\.(0[1-9]|1[0-2])\.(0[1-9]|[12][0-9]|3[01])', s)
        date_obj = datetime.strptime(date_match.group(), '%Y.%m.%d').date()

        class_match = re.search(r'"([1-5]-(?:0[1-9]|1[0-9]|20))"', s)
        class_str = class_match.group(1) if class_match else "Не указана"

        teacher_match = re.search(r'"([а-яА-ЯёЁ\s\.]{2,})"', s)
        teacher_str = teacher_match.group(1) if teacher_match else "Не указан"
...
def find_conflicts(lessons_list):
...
try:
    with open(filename, 'r', encoding='utf-8') as file:
        print(f"[ИНФО] Чтение файла '{filename}'...")
        for line in file:
            if line.strip():
    ...
    print(f"ОШИБКА: Файл '{filename}' не найден. Пожалуйста, создайте его или проверьте путь.")
    exit()
